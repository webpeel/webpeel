...
describe('Firecrawl compatibility routes', () => {
    ...
    describe('POST /v1/scrape', () => {
        ...
        it('handles onlyMainContent parameter', async () => {
            ...
            expect(mockPeel).toHaveBeenCalledWith('https://example.com', expect.objectContaining({ raw: true }));
        });
    ...
    describe('POST /v1/crawl', () => {
        it('returns job ID', async () => {
            mockJobQueue.createJob.mockReturnValue({
                id: 'job-123',
                type: 'crawl',
                status: 'queued',
                createdAt: Date.now(),
    ...
    describe('POST /v1/map', () => {
        it('returns links array', async () => {
            mockMapDomain.mockResolvedValue({
                urls: [
        ...
        it('handles limit parameter', async () => {
            ...
            expect(mockMapDomain).toHaveBeenCalledWith('https://example.com', expect.objectContaining({ maxUrls: 100 }));
        });
        it('handles search parameter', async () => {
            ...
            expect(mockMapDomain).toHaveBeenCalledWith('https://example.com', expect.objectContaining({ search: 'blog' }));
        });
        it('handles map errors gracefully', async () => {
            mockMapDomain.mockRejectedValue(new Error('Map failed'));
            const response = await request(app)
                .post('/v1/map')
                .send({ url: 'https://example.com' });
            expect(response.status).toBe(500);
    ...
    describe('Error response format', () => {
        it('matches Firecrawl error format', async () => {
            mockPeel.mockRejectedValue(new Error('Test error'));
            const response = await request(app)
                .post('/v1/scrape')
                .send({ url: 'https://example.com' });
