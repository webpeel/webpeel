...
describe('runAgent', () => {
    ...
    it('handles network errors during page fetching', async () => {
        ...
        // Mock peel failure
        mockPeel.mockRejectedValue(new Error('Network error'));
        const _result = await runAgent({
            prompt: 'Test prompt',
            llmApiKey: 'test-key',
            maxPages: 1,
    ...
    it('uses custom LLM model when specified', async () => {
        ...
        const llmCall = fetchCalls.find((call) => call[0].includes('chat/completions'));
    ...
    it('uses custom API base URL when specified', async () => {
        ...
        const fetchCalls = mockFetch.mock.calls;
        const llmCall = fetchCalls.find((call) => call[0].includes('custom-api.com'));
        expect(llmCall).toBeDefined();
    });
    ...
    it('uses provided URLs instead of searching', async () => {
        ...
        const fetchCalls = mockFetch.mock.calls;
        const searchCalls = fetchCalls.filter((call) => call[0].includes('duckduckgo'));
    ...
    it('includes sources in result', async () => {
        ...
        mockPeel.mockImplementation(async (url) => ({
            url,
            title: 'Test',
            content: 'Content',
            method: 'simple',
