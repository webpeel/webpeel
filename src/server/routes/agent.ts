...
export function createAgentRouter(): Router {
  ...
  /**
   * POST /v1/agent - DEPRECATED (410 Gone)
   *
   * Use /v1/fetch with the webpeel MCP tool instead.
   */
  router.post('/v1/agent', (_req: Request, res: Response) => {
    res.status(410).json({
      error: 'gone',
      message: 'Use /v1/fetch with the webpeel MCP tool instead',
    });
  });
